    ...
    <b-card no-body>
      <b-card-header :header-bg-variant="rating.reviewrequired ? 'warning' : 'default'" class="d-flex justify-content-between flex-wrap">
        <div>
          ...
          <nuxt-link :to="'/modtools/members/approved/search/' + rating.groupid + '/' + rating.rater"><strong>{{ rating.raterdisplayname }}</strong> (<v-icon icon="hashtag" class="text-muted" scale="0.75" />{{ rating.rater }})</nuxt-link>
          ...
          <!-- eslint-disable-next-line-->
          <nuxt-link :to="'/modtools/members/approved/search/' + rating.groupid + '/' + rating.ratee"><strong>{{ rating.rateedisplayname }}</strong> (<v-icon icon="hashtag" class="text-muted" scale="0.75" />{{ rating.ratee }})</nuxt-link>
        </div>
        <div>
