      ...
      >
        ...
        <div class="d-flex">
          <div v-if="chat.chattype === 'User2User' && otheruser" class="mr-2">
            ...
            <template v-else>
              <b-button
                v-b-tooltip="'Block this member from talking to you.'"
                variant="secondary"
                class="d-none d-md-block"
                @click="showblock"
              >
...
// Set up chat data using MT-specific composable
const { chat, otheruser, unseen, milesaway, milesstring } = await setupChatMT(
  props.id
)
...
/* MT ChatHeader: Always visible (no display:none on small screens) */
.outer {
