  ...
  <div class="scrollinplace">
    ...
    <div v-else-if="configid && config">
        ...
        <div v-if="showUsing">
          <div v-for="using in config.using" :key="'using-' + using.id">
            ...
            <span class="text-muted small">
              <v-icon icon="hashtag" class="text-muted" scale="0.75" />{{ using.userid }}
            </span>
          </div>
        </div>
      </NoticeMessage>
      <b-card no-body class="mb-2 mt-1">
        <b-card-header>
          <b-button v-b-toggle.accordion-general block href="#" variant="secondary">
            General Settings
          </b-button>
        </b-card-header>
        <b-collapse id="accordion-general" accordion="settings-accordion" role="tabpanel">
          <b-card-body>
            ...
            <ModConfigSetting :configid="configid" name="name" label="Name"
              ...
              :disabled="locked" />
            <ModConfigSetting :configid="configid" name="chatread" label="Leave chat unread?"
              ...
              type="toggle" toggle-checked="Mark as read" toggle-unchecked="Leave as unread" :disabled="locked" />
            <ModConfigSetting :configid="configid" name="fromname" label="'From:' name in messages"
              description="You can choose whether the mod's own name is used in standard messages." type="toggle" value-checked="My name"
              value-unchecked="Groupname Moderator" toggle-checked="Own Name" toggle-unchecked="$groupname Moderator" :toggle-width="200"
              :disabled="locked" />
            <ModConfigSetting :configid="configid" name="coloursubj" label="Colour-code subjects?"
              description="Whether subjects are coded green/red based to indicate that they are correctly formatted." type="toggle"
              toggle-checked="Yes" toggle-unchecked="No" :disabled="locked" />
            <ModConfigSetting :configid="configid" name="subjlen" label="Subject length warning"
              description="Keeping subject lines short is better for small screens." :disabled="locked" />
            <ModConfigSetting :configid="configid" name="subjreg" label="Regular expression for colour-coding"
              description="Regular expressions can be difficult; test changes at http://www.phpliveregex.com" :disabled="locked" />
            <ModConfigSetting :configid="configid" name="network" label="$network substitution string"
              description="Normally you'd leave this set to Freegle or blank (same thing)." :disabled="locked" />
            <ModConfigSetting v-if="config.createdby" :configid="configid" name="protected" label="Locked against changes?"
              description="You can set this read-only so that only the person who created it can make changes" type="toggle" toggle-checked="Locked"
              toggle-unchecked="Unlocked" :disabled="locked" />
          </b-card-body>
        </b-collapse>
      </b-card>
      <b-card no-body class="mb-2 mt-1">
        <b-card-header>
          <b-button v-b-toggle.accordion-pendingmessages block href="#" variant="secondary">
            Pending Messages
          </b-button>
        </b-card-header>
        <b-collapse id="accordion-pendingmessages" accordion="settings-accordion" role="tabpanel">
          <b-card-body>
            <ModSettingsStandardMessageSet cc="ccrejectto" addr="ccrejectaddr"
              :types="['Approve', 'Reject', 'Leave', 'Delete', 'Edit', 'Hold Message']" :locked="locked" />
          </b-card-body>
        </b-collapse>
      </b-card>
      <b-card no-body class="mb-2 mt-1">
        <b-card-header>
          <b-button v-b-toggle.accordion-approvedmessages block href="#" variant="secondary">
            Approved Messages
          </b-button>
        </b-card-header>
        <b-collapse id="accordion-approvedmessages" accordion="settings-accordion" role="tabpanel">
          <b-card-body>
            <ModSettingsStandardMessageSet cc="ccfollowupto" addr="ccfollowupaddr"
              :types="['Leave Approved Message', 'Delete Approved Message', 'Edit']" :locked="locked" />
          </b-card-body>
        </b-collapse>
      </b-card>
      ...
      <b-card no-body class="mb-2 mt-1">
        <b-card-header>
          <b-button v-b-toggle.accordion-approvedmembers block href="#" variant="secondary">
            Approved Members
          </b-button>
        </b-card-header>
        <b-collapse id="accordion-approvedmembers" accordion="settings-accordion" role="tabpanel">
          <b-card-body>
            <ModSettingsStandardMessageSet cc="ccfollmembto" addr="ccfollmembaddr" :types="['Leave Approved Member', 'Delete Approved Member']"
              :locked="locked" />
          </b-card-body>
        </b-collapse>
      </b-card>
      ...
      <ConfirmModal v-if="showDeleteModal" :title="'Delete: ' + config.name" @confirm="deleteConfirmed" @hidden="showDeleteModal = false" />
    </div>
  </div>
</template>
<script>
