<template>
  <div>
    <h2>
      Active Volunteers <span class="text-muted">on {{ groupName }}</span>
    </h2>
    <div v-if="groupid">
      ...
      <div v-else-if="ModeratorsActive && ModeratorsActive.length">
        <p>
          This shows when the volunteers on this community last moderated on
          ModTools. Just visiting the site isn't enough to cause this to update.
        </p>
        <b-card no-body>
          <b-card-body>
            <b-row
              v-for="user in ModeratorsActiveVisible"
              :key="'popular-' + user.id"
            >
            ...
            <b-button
              v-if="ModeratorsActive.length > 10 && !ModeratorsExpanded"
              variant="link"
              @click="ModeratorsExpanded = true"
            >
              Show more...
            </b-button>
            <b-button
              v-if="ModeratorsActive.length > 10 && ModeratorsExpanded"
              variant="link"
              @click="ModeratorsExpanded = false"
            >
              Show fewer...
            </b-button>
          </b-card-body>
    ...
    <div v-else>
      <p>
        Select a community and click <em>Update</em> to show the active
        volunteers for that community.
      </p>
    </div>
  </div>
</template>
