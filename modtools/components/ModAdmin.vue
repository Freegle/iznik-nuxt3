<template>
  <div>
      ...
      <b-card-body v-if="expanded">
        ...
        <p class="text-muted">
          ...
          <span v-else-if="admin.createdby">
            ...
            <span class="text-muted small">
              <v-icon icon="hashtag" class="text-muted" scale="0.75" /> {{ admin.createdby.id }}
            </span>
          </span>
        </p>
        ...
        <b-form-group label="Subject of ADMIN:" label-for="subject" label-class="mb-0">
          <b-form-input id="subject" v-model="admin.subject" class="mb-3" placeholder="Subject (don't include ADMIN - added automatically)" />
        </b-form-group>
        <b-form-group label="Body of ADMIN:" label-for="body" label-class="mb-0">
          <b-form-textarea id="body" v-model="admin.text" class="mb-3" placeholder="Put your message in here.  Plain-text only." rows="15" />
        </b-form-group>
        <b-form-group label="Call To Action text:" label-for="ctatext" label-class="mb-0">
          <b-form-input id="ctatext" v-model="admin.ctatext" class="mb-3" placeholder="(Option) Text for a big button" />
        </b-form-group>
        <b-form-group label="Call To Action link:" label-for="ctalink" label-class="mb-0">
          <b-form-input id="ctalink" v-model="admin.ctalink" class="mb-3" placeholder="(Optional) Link for a big button" />
        </b-form-group>
      </b-card-body>
    ...
    <ConfirmModal v-if="showConfirmModal" :title="'Delete: ' + admin.subject" @confirm="deleteConfirmed" @hidden="showConfirmModal = false" />
  </div>
</template>
<script>
  ...
  computed: {
    ...
    holder() {
      return this.admin.heldby
        ? this.userStore.byId(this.admin.heldby)
        : null
    }
  },
  mounted() {
