      ...
      <template #default>
        <div v-if="alert">
          ...
          <div v-else>
            ...
            <h2>Group Stats</h2>
            <p>
              Sent to {{ pluralise('group', alert.stats.responses.groups.length, true) }}.
            </p>
            <GChart type="PieChart" :data="groupData" :options="chartOptions" />
            <h2>Volunteer Stats</h2>
            <p>
              Sent to {{ pluralise('volunteer', alert.stats.sent.mods, true) }}
              via {{ pluralise(['email address', 'email addresses'], alert.stats.sent.modemails, true) }}.
            </p>
            <GChart type="PieChart" :data="volunteerData" :options="chartOptions" />
          </div>
        </div>
      </template>
      <template #footer>
