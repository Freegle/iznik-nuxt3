  ...
  <div v-if="!hide">
    <b-row>
      <b-col cols="6" md="3" class="pl-3">
        <div>
          <b-form-input v-model="editgiftaid.fullname" :class="{ 'border-danger': nameInvalid }" />
          <div v-if="email">
          ...
          <span class="small text-muted">
            ...
            User ID <v-icon icon="hashtag" class="text-muted" scale="0.75" />{{ editgiftaid.userid }}
            Gift Aid ID <v-icon icon="hashtag" class="text-muted" scale="0.75" />{{ editgiftaid.id }}
          </span>
        </div>
      </b-col>
      <b-col cols="6" md="5">
        <b-form-textarea v-model="editgiftaid.homeaddress" rows="4" />
        <b-form-input v-model="editgiftaid.housenameornumber" :class="{ 'border-danger': houseInvalid, 'mt-1': true }"
          placeholder="House name or number" />
        <b-form-input v-model="editgiftaid.postcode" :class="{ 'border-danger': postcodeInvalid, 'mt-1': true }" placeholder="Postcode" />
      </b-col>
      <b-col cols="6" md="4" class="">
        ...
        <SpinButton variant="success" icon-name="check" label="Looks Good" @handle="reviewed"
          :disabled="houseInvalid || postcodeInvalid || nameInvalid || !editgiftaid.donations" />
      </b-col>
    </b-row>
    <b-row>
      <b-col>
...
export default {
  ...
  computed: {
    ...
    postcodeInvalid() {
      if( !this.editgiftaid) return false
      return !this.editgiftaid?.postcode || this.editgiftaid?.postcode.indexOf(' ') === -1
    },
    houseInvalid() {
      if( !this.editgiftaid) return false
