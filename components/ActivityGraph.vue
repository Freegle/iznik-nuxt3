  ...
  <b-card variant="white" class="mt-2">
    <b-card-text>
      ...
      <p v-if="graphType === 'Donations'">
        This is the sum in pounds of donations received via PayPal or Stripe.
      </p>
...
export default {
  ...
  computed: {
    ...
    duration() {
      const d = dayjs(this.end)
        .endOf('day')
        .diff(dayjs(this.start).startOf('day'), 'day')
      return d
    },
    graphData() {
      ...
      const data = []
      const startd = dayjs(this.start).startOf('day')
      const endd = dayjs(this.end).endOf('day')
