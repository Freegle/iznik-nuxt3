...
describe('DonationAskButtons51025', () => {
  function createWrapper(props = {}) {
    return mount(DonationAskButtons51025, {
      props: {
  ...
  describe('rendering', () => {
    ...
    it('has £25 button', () => {
      const wrapper = createWrapper()
      expect(wrapper.find('.donation-btn[data-value="25"]').exists()).toBe(true)
    })
  ...
  describe('events', () => {
    ...
    it('emits score 25 when £25 button clicked', async () => {
      const wrapper = createWrapper()
      await wrapper.find('.donation-btn[data-value="25"]').trigger('click')
      expect(wrapper.emitted('score')[0]).toEqual([25])
    })
  })
